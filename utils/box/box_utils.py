...
def bbox_overlaps_ciou(bboxes1, bboxes2):
    ...
    w1 = torch.clamp(w1, min=min_values)
    h1 = torch.clamp(h1, min=min_values)
    w2 = torch.clamp(w2, min=min_values)
    h2 = torch.clamp(h2, min=min_values)
    ...
    inter = torch.clamp((inter_max_xy - inter_min_xy), min=min_values)
    inter_area = inter[:, 0] * inter[:, 1]
    inter_diag = (center_x2 - center_x1)**2 + (center_y2 - center_y1)**2
    outer = torch.clamp((out_max_xy - out_min_xy), min=min_values)
    outer_diag = (outer[:, 0] ** 2) + (outer[:, 1] ** 2)
    union = area1+area2-inter_area
    ...
    with torch.no_grad():
        ...
        alpha = v / (S + v)
    cious = iou - (u + alpha * v)
    # tmp_cious = torch.tensor(cious)
    # tmp_cious = tmp_cious.cpu().numpy()
    # if np.any(np.isnan(tmp_cious)):
    #     print(1)
