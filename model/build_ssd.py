...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, Backbone, Neck, Head, cfg):
        ...
        self.detect = Detect(self.num_classes, 0, 600, 0.001, 0.4,variance = cfg['variance'], nms_kind=cfg['nms_kind'], beta1=cfg['beta1'])
...
def build_ssd(phase, size=300, cfg=None):
    ...
    if size != 300 and size!=600 and size!=800 and size != 512:
        ...
        return
    print(phase)
    # base = Backbone(cfg['model'], [5, 6, 7, 8, 9, 10, 11], phase)
    base = Backbone(cfg['model'], cfg['feature_maps'], phase)
    # base = Backbone(cfg['model'], [6, 7, 8, 9, 10, 11], phase)
