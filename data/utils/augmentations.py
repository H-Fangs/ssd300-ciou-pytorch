...
class ToPercentCoords(object):
    ...
    def __call__(self, image, boxes=None, labels=None):
        ...
        return image, boxes.astype(np.float), labels
...
class RandomRotate90(object):
    ...
    def __call__(self, image, boxes, classes):
        ...
        if random.random() < 0.5:
            ...
            x = copy.deepcopy(boxes[:, 0])
            y = copy.deepcopy(boxes[:, 1])
            w = copy.deepcopy(boxes[:, 2] - boxes[:, 0])
            h = copy.deepcopy(boxes[:, 3] - boxes[:, 1])
            cx = copy.deepcopy((boxes[:, 2] + boxes[:, 0]) // 2)
            cy = copy.deepcopy((boxes[:, 3] + boxes[:, 1]) // 2)
